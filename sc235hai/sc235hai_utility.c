 ...
 * All rights reserved.
 ***************************************************************************/
#define pr_fmt(fmt)             "[sc235hai]:" fmt
...
#include "inc/sc235hai_setting.h"
#include "inc/sensor_effect_common.h"
#include "hb_camera_data_config.h"


int sc235hai_linear_data_init(sensor_info_t *sensor_info);
...
int sensor_init(sensor_info_t *sensor_info)
{
        ...
        switch(sensor_info->sensor_mode) {
                case NORMAL_M:  // 1: normal
                        vin_info("sc235hai in normal linear mode\n");
                        vin_info("bus_num = %d, sensor_addr = 0x%0x\n", sensor_info->bus_num, sensor_info->sensor_addr);

                        setting_size = sizeof(sc235hai_linear_30fps_init_setting) / sizeof(uint32_t) / 2;
                        ret = vin_write_array(sensor_info->bus_num, sensor_info->sensor_addr, 2,
                                                    setting_size, sc235hai_linear_30fps_init_setting);
                        if (ret < 0) {
                                vin_err("%d : init %s fail\n", __LINE__, sensor_info->sensor_name);
                                return -HB_CAM_I2C_WRITE_FAIL;
                        }
                        ret = sc235hai_linear_data_init(sensor_info);
                        if (ret < 0) {
        ...
        vin_info("sc235hai config success under %d mode\n", sensor_info->sensor_mode);
...
int sensor_start(sensor_info_t *sensor_info)
{
        ...
        switch(sensor_info->sensor_mode) {
                case NORMAL_M:
                case SLAVE_M:
                        setting_size = sizeof(sc235hai_stream_on_setting)/sizeof(uint32_t)/2;
                        vin_info("%s start normal / slave linear mode\n", sensor_info->sensor_name);
                        ret = vin_write_array(sensor_info->bus_num, sensor_info->sensor_addr, 2,
                                        setting_size, sc235hai_stream_on_setting);
                        if(ret < 0) {
...
int sensor_stop(sensor_info_t *sensor_info)
{
        ...
        setting_size =
                sizeof(sc235hai_stream_off_setting) / sizeof(uint32_t) / 2;
        vin_info("%s sensor stop\n", sensor_info->sensor_name);
        ret = vin_write_array(sensor_info->bus_num,
                        sensor_info->sensor_addr, 2,
                        setting_size, sc235hai_stream_off_setting);
        if (ret < 0) {
                vin_err("start %s fail\n", sensor_info->sensor_name);
                return -HB_CAM_I2C_WRITE_FAIL;
        }
...
int sc235hai_linear_data_init(sensor_info_t *sensor_info)
{
        ...
        //read vts = frame_length = HMAX, default value is 0x04b0 = 1200
...
#endif
        turning_data.sensor_data.lines_per_second = 36000;//vts * fps, should be fixed = 1200 * 30
        //lines_per_second/fps = vts
        // from customer, max 10ms
        // 1000ms -lines_per_second - 36000
        // 10ms - 400, 33ms - 1200
        turning_data.sensor_data.exposure_time_max = 1200; //from customer, max 33ms
	...
	turning_data.sensor_data.exposure_time_init = 377;
        ...
        if(sizeof(turning_data.stream_ctrl.stream_on) >= sizeof(sc235hai_stream_on_setting)) {
                memcpy(stream_on, sc235hai_stream_on_setting, sizeof(sc235hai_stream_on_setting));
        } else {
                vin_err("Number of registers on stream over 10\n");
                return -RET_ERROR;
        }
        if(sizeof(turning_data.stream_ctrl.stream_off) >= sizeof(sc235hai_stream_off_setting)) {
                memcpy(stream_off, sc235hai_stream_off_setting, sizeof(sc235hai_stream_off_setting));
        } else {
                vin_err("Number of registers on stream over 10\n");
                return -RET_ERROR;
        }
        ...
        if (turning_data.normal.again_lut != NULL) {
                ...
                memcpy(turning_data.normal.again_lut, sc235hai_gain_lut,
                        sizeof(sc235hai_gain_lut));
        }
...
static int sensor_aexp_gain_control(hal_control_info_t *info, uint32_t mode, uint32_t *again, uint32_t *dgain, uint32_t gain_num)
{
        ...
        if (mode == NORMAL_M || mode == SLAVE_M) {
                if (again[0] >= sizeof(sc235hai_gain_lut)/sizeof(uint32_t))
                        gain_index = sizeof(sc235hai_gain_lut)/sizeof(uint32_t) - 1;
                else
                        gain_index = again[0];

                again_reg_value = (sc235hai_gain_lut[gain_index] >> 24) & 0x000000FF;
                a_fine_gain_reg_value = (sc235hai_gain_lut[gain_index] >> 16) & 0x000000FF;
                dgain_reg_value = (sc235hai_gain_lut[gain_index] >> 8) & 0x000000FF;
                d_fine_gain_reg_value = sc235hai_gain_lut[gain_index] & 0x000000FF;
...
static int32_t sensor_update_fps_notify_driver(sensor_info_t *sensor_info)
{
        ...
        switch(sensor_info->sensor_mode) {
                ...
                case (uint32_t)SLAVE_M:
                        ret = sc235hai_linear_data_init(sensor_info);
                        if (ret < 0) {
                                vin_err("update fps sc235hai_linear_data_init fail\n");
                                return ret;
                        }
                        break;
...
{
        int32_t ret = RET_OK;
        const uint16_t SC235HAI_VTS_LOW = 0x320f;
        const uint16_t SC235HAI_VTS_HIGH = 0x320e;
        int32_t vts;
        ...
        ret = hb_vin_i2c_write_reg16_data8(sensor_info->bus_num, sensor_info->sensor_addr,
                        SC235HAI_VTS_HIGH, ((vts >> 8) & 0x7f)); //0x320e[0:6] 0x0d for 10fps
        ret |= hb_vin_i2c_write_reg16_data8(sensor_info->bus_num, sensor_info->sensor_addr,
                        SC235HAI_VTS_LOW, (vts & 0xff) - 2); // 减小2行加点裕度 0x2d for 10fps
        if (ret < 0) {
...
SENSOR_MODULE_F(sc235hai, CAM_MODULE_FLAG_A16D8);
sensor_module_t sc235hai = {
        .module = SENSOR_MNAME(sc235hai),
#else
sensor_module_t sc235hai = {
        .module = "sc235hai",
#endif
